...
library(readxl)
anes_imputed <- read_csv("~/Downloads/df_imputed.csv")
...
anes_imputed %>%
  ...
  mutate(rowname = reorder(rowname, strongleader)) %>%
  ggplot(aes(rowname, strongleader)) +
  geom_col() +
  coord_flip() +
  labs(y = "Strong_leader", 
...
anes_imputed %>%
  ...
  correlate(use = "pairwise.complete.obs",
            method = "pearson",
            quiet = TRUE) %>% 
  focus(trust_religiousauthority) %>%
  rownames_to_column(var = "rowname") %>%       
  mutate(rowname = reorder(rowname, trust_religiousauthority)) %>%
  ggplot(aes(rowname, trust_religiousauthority)) +
  geom_col() + 
  coord_flip() + 
  labs(y = "trust_religiousauthority", 
       x = "All Other Feeling Thermometers") + 
  theme_minimal() +
